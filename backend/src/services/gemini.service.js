...
/**
 * Calls OpenRouter to compute a compatibility score.
 *
...
async function getCompatibilityScore(tenantProfile, listing) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn('[openrouter] API returned error:', response.status, errorText);
      return null;
    }
    ...
    if (!text) {
      console.warn('[openrouter] No content in response:', data);
      return null;
    }
    ...
    if (isNaN(score) || score < 0 || score > 100) {
      console.warn('[openrouter] Invalid score value:', parsed.score);
      return null;
    }
    ...
    console.warn('[openrouter] Score computation failed, will use fallback:', err.message);
    return null;
  }
}
...
TENANT PREFERENCES:
- Looking for: ${tenantProfile.intent === 'ENTIRE_PROPERTY' ? 'An entire property to rent' : 'A roommate / flatmate vacancy'}
...
LISTING:
- Type: ${listing.intent === 'ENTIRE_PROPERTY' ? 'Entire Property' : 'Flatmate Vacancy / Room Available'}
- Title: ${listing.title}
